...
def CreateProjectFolder(ExName, ExAdr, targetLabel, model_name, repeat = None):
    ...
    outputPath = ExAdr.split(os.path.sep)
    outputPath = outputPath[:-1]
    outputPath[0] = outputPath[0] + os.path.sep
...
def get_optim(model, args, params = False):
    ...
    if args.opt == "adam":
        ...
        optimizer = optim.AdamW(temp, lr = args.lr, weight_decay = args.reg)
    elif args.opt == 'sgd':
...
def safe_log(message, reportFile=None, rank=0):
    """
    Log a message safely across ranks. Writes to the reportFile if provided; otherwise, prints to console.
    """
    if reportFile:
        reportFile.write(f"[Rank {rank}] {message}\n")
    else:
        print(f"[Rank {rank}] {message}")

    # Synchronize to avoid overlapping writes
...
def ReadExperimentFile(args, deploy = False):
    ...
    args.project_name = args.adressExp.split(os.path.sep)[-1].replace('.txt', '')  
    
    try:
        if not deploy :
            datadir_train = data['dataDir_train']
    except:
        raise NameError('TRAINING DATA ADDRESS IS NOT DEFINED!')
    print("Deploy: ", deploy)           
    args.clini_dir = []
    args.slide_dir = []
    args.datadir_train = []
    ...
    #     #print("Finetuning of previous model.\n")
    #     print("Model Adr: ", args.modelAdr)
    # except:
    #     print('MODEL ADDRESS IS NOT DEFINED! \nDEFAULT VALUE WILL BE USED : None\n')  
    #     print('-' * 30)
    #     args.modelAdr = None
    
    if not deploy :
        for index, item in enumerate(datadir_train):
            print("Checking for clinical data at:", os.path.join(item, item.split(os.path.sep)[-1] + '_CLINI.xlsx'))
            ...
            if not deploy:
                if os.path.isfile(os.path.join(item, item.split(os.path.sep)[-1] + '_CLINI.xlsx')):
                     args.clini_dir.append(os.path.join(item, item.split(os.path.sep)[-1] + '_CLINI.xlsx'))
                else:
                     raise NameError('NO CLINI DATA FOR ' + item + ' IS FOUND!')
    
                if os.path.isfile(os.path.join(item, item.split(os.path.sep)[-1] + '_SLIDE.xlsx')):
                     args.slide_dir.append(os.path.join(item, item.split(os.path.sep)[-1] + '_SLIDE.xlsx'))
                else:
    ...
    if deploy:
        ...
        for index, item in enumerate(datadir_test):
            ...
            # Check for both CSV and XLSX slide files
            slide_csv_path = os.path.join(item, item.split(os.path.sep)[-1] + '_SLIDE.csv')
            slide_xlsx_path = os.path.join(item, item.split(os.path.sep)[-1] + '_SLIDE.xlsx')
            
            if os.path.isfile(slide_csv_path):
                args.slide_dir.append(slide_csv_path)
            elif os.path.isfile(slide_xlsx_path):
                args.slide_dir.append(slide_xlsx_path)
            else:
                raise NameError('NO SLIDE DATA FOR ' + item + ' IS FOUND!')           
    ...
    cnn_models = {'resnet18', 'resnet50', 'vit', 'efficient', 'densenet'}
    if args.model_name not in cnn_models:
        raise ValueError(
            f"Unsupported modelName '{args.model_name}'. "
            f"This HIA package supports CNN training only: {sorted(cnn_models)}"
        )

    args.useClassicModel = True
    print("CNN-only mode: useClassicModel =", args.useClassicModel)
    try:
        args.train_full = MakeBool(data['trainFull'])
        print("Train Full: ", args.train_full)
    except:
    ...
    try:
        ...
        print('EARLY STOPPING VALUE IS NOT DEFINED!\nDEFAULT VALUE WILL BE USED : TRUE\n')  
        print('-' * 30)
...
def MakeBool(value):
    """
    Convert various representations of boolean values to a Python boolean.
    Accepts 'True', 'true', '1' as True, and 'False', 'false', '0' as False.
    """
    if isinstance(value, str):
        ...
        if value in ['true', '1', 't']:
            ...
            return False
    return bool(value)  # Convert other types to boolean
