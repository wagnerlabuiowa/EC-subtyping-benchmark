...
def Train_MIL_CLAM(datasets, fold, args, trainFull = False):
    raise NotImplementedError(
        "MIL/CLAM training has been removed from this public HIA package. "
        "Use Classic_Training_DDP.py for CNN training."
    )
...
def Train_model_Classic(model, trainLoaders, args, valLoaders = [], criterion = None, optimizer = None, fold = False, rank = 0):    
    ...
    print("args.early_stopping: ", args.early_stopping)
    if args.early_stopping:
        print("Early stopping is enabled")
        early_stopping = EarlyStopping(patience = args.patience, stop_epoch = args.minEpochToTrain, verbose = True)    
    for epoch in range(args.max_epochs):
        phase = 'train'
        print('Train: Epoch {}/{} on GPU {}\n'.format(epoch, args.max_epochs - 1, rank))       
        model.train() 
        running_loss = 0.0
        running_corrects = 0
        total_samples = 0 # to keep track of total samples processed in DDP
        ...
        for inputs, labels in tqdm(trainLoaders):
            ...
            with torch.set_grad_enabled(True):
                ...
                total_samples += inputs.size(0) # increment total samples processed in DDP
        dist.barrier()        
        ...
        print('\nGPU {}: {} Loss: {:.4f} Acc: {:.4f}'.format(rank, phase, epoch_loss, epoch_acc))
        dist.barrier()   
        #Validation Phase
        if valLoaders:
            ...
            print('\nGPU {}: {} Loss: {:.4f} Acc: {:.4f}'.format(rank, phase, val_loss, val_acc)) 
            ...
            if early_stopping is not None:
                early_stopping(epoch, val_loss, model, ckpt_name = ckpt_name)
                # Gather early stopping status from all GPUs
                early_stopping_status = torch.tensor(early_stopping.early_stop, dtype=torch.bool).to(device)
                gathered_status = [torch.tensor(0, dtype=torch.bool).to(device) for _ in range(dist.get_world_size())]
                dist.all_gather(gathered_status, early_stopping_status)
                if all(gathered_status):
                #if early_stopping.early_stop:
                    print('-' * 30)
                    #print("The Validation Loss Did Not Decrease -- Early Stopping.")
                    print("Early stopping is triggered on all GPUs.Exiting training.")
                    print('-' * 30)
                    break
            print('-' * 30)
        dist.barrier()
...
def Train_model_AttMIL(model, trainLoaders, args, valLoaders = [], criterion = None, optimizer = None, fold = False):    
    raise NotImplementedError(
        "AttMIL training has been removed from this public HIA package."
    )
...
def Validate_model_AttMIL(model, dataloaders):
    raise NotImplementedError(
        "AttMIL validation has been removed from this public HIA package."
    )
