...
class DatasetLoader_Classic(torch.utils.data.Dataset):
    ...
    def __init__(self, imgs, labels, transform = None, target_patch_size = -1):
        ...
        self.invalid_indices = set()  # Initialize the invalid_indices attribute
    ...
    def __getitem__(self, index):
        try:
            ...
            print(f"Warning: Skipping corrupted image {self.imgs[index]} - {e}")
            self.invalid_indices.add(index) # Mark index as invalid
            return None 
            
# # Custom collate function to filter out None
# def custom_collate_fn(batch):
#     batch = [item for item in batch if item is not None]  # Remove None items
#     if len(batch) == 0:
#         return None  # Handle cases where the entire batch is None
#     return torch.utils.data.default_collate(batch)

    # def remove_corrupted(self):
    #     # Remove all corrupted images before using the dataset
    #     valid_imgs = []
    #     valid_labels = []
    #     print("Checking for corrupted images...")
    #     for i in tqdm(range(len(self.imgs)), desc="Validating Images"):
    #         try:
    #             Image.open(self.imgs[i]).convert('RGB')  # Test if the image is valid
    #             valid_imgs.append(self.imgs[i])
    #             valid_labels.append(self.labels[i])
    #         except (FileNotFoundError, PIL.UnidentifiedImageError):
    #             print(f"Skipping corrupted image: {self.imgs[i]}")
    #     self.imgs = valid_imgs
    #     self.labels = valid_labels
    #     print(f"Removed corrupted images. Total valid images: {len(self.imgs)}")
###############################################################################
