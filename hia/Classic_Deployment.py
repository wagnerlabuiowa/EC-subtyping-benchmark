...
if __name__ == '__main__':
    ...
    # for batch in tqdm(testGenerator, desc="Processing Dataloader"):
    #     if batch is None:
    ...
    try:
        state_dict = torch.load(args.modelAdr)
        if isinstance(state_dict, collections.OrderedDict):
            # If we loaded a state dictionary
            model.load_state_dict(state_dict)
        else:
            # If we loaded a full model
            model = state_dict
    ...
    model.to(device)
    if device.type == 'cuda':
        # Convert to mixed precision for faster inference
        model = model.half()
        # Define half precision transform function
    ...
    # Use the transform function directly (not as a class reference)
