...
class FeatureExtractorUNI:
    def init_feat_extractor(self, device: str, **kwargs):
        ...
        model_path = os.path.join(asset_dir, "pytorch_model.bin")
        ...
        digest = get_digest(f"{asset_dir}/pytorch_model.bin")
...
class FeatureExtractorUNI2:
    def init_feat_extractor(self, device: str, **kwargs):
        ...
        digest = get_digest(f"{asset_dir}/pytorch_model.bin")
        model_name = f"mahmood-uni2-h-{digest[:8]}"
...
class FeatureExtractorVirchow2:
    def init_feat_extractor(self, device: str, **kwargs):
        asset_dir = f"{os.environ['STAMP_RESOURCES_DIR']}/virchow2"
        model_path = os.path.join(asset_dir, "pytorch_model.bin")
        ...
        digest = get_digest(f"{asset_dir}/pytorch_model.bin")
        model_name = f"paige-ai-virchow2-{digest[:8]}"

        print("Virchow2 model successfully initialised...\n")
        return model_name

class FeatureExtractorHOptimus0:
    def init_feat_extractor(self, device: str, **kwargs):
        asset_dir = f"{os.environ['STAMP_RESOURCES_DIR']}/h-optimus-0"
        model_path = os.path.join(asset_dir, "pytorch_model.bin")
        ...
        digest = get_digest(f"{asset_dir}/pytorch_model.bin")
        model_name = f"bioptimus-h-optimus-0-{digest[:8]}"  

        print("H-Optimus-0 model successfully initialised...\n")
        return model_name

class FeatureExtractorProvGigaPath:
    def init_feat_extractor(self, device: str, **kwargs):
        asset_dir = f"{os.environ['STAMP_RESOURCES_DIR']}/prov-gigapath"
        model_path = os.path.join(asset_dir, "pytorch_model.bin")
        ...
        digest = get_digest(f"{asset_dir}/pytorch_model.bin")
        model_name = f"prov-gigapath-{digest[:8]}"

        print("Prov-GigaPath model successfully initialised...\n")
        return model_name

class FeatureExtractorViTB16:
    def init_feat_extractor(self, device: str, **kwargs):
        asset_dir = f"{os.environ['STAMP_RESOURCES_DIR']}/ViT-B16"
        model_path = os.path.join(asset_dir, "pytorch_model.bin")
        ...
        digest = get_digest(f"{asset_dir}/pytorch_model.bin")
        model_name = f"ViT-B16-{digest[:8]}"

        print("ViT-B16 model successfully initialised...\n")
        return model_name
...
def extract_features_(
    ...
    with open(outdir.parent/'info.json', 'w') as f:
        json.dump({'extractor': extractor_string,
                  ...
                  'mpp': target_microns/patch_size}, f)
    ...
    dl = torch.utils.data.DataLoader(
        ds, batch_size=16, shuffle=False, num_workers=cores, drop_last=False, pin_memory=device != 'cpu')
    ...
    with h5py.File(f'{outdir}.h5', 'w') as f:
        # datasets -----------------------------------------------------------
        f.create_dataset('coords', data=np.asarray(coords), compression="gzip")
        f.create_dataset('feats', data=torch.concat(feats).cpu().numpy(), compression="gzip")
        f.create_dataset('augmented', data=np.repeat([False, True], [len(unaugmented_ds), len(augmented_ds)]), compression="gzip")
        assert len(f['feats']) == len(f['augmented'])
        f.attrs['extractor'] = extractor_string
        f.attrs['patch_size'] = patch_size
        f.attrs['mpp'] = target_microns/patch_size
