...
class Normalizer(object):
    ...
    def transform(
        self,
        og_img: np.ndarray,
        bg_rejected_img: np.ndarray,
        rejected_list: np.ndarray,
        patch_shapes: list,
        target_mpp: float,
        cores: int = 8,
    ):
        """
        Patch-wise Macenko normalisation.

        Parameters
        ----------
        og_img          : RGB ndarray of the whole slide (down-sampled)
        bg_rejected_img : ndarray with background pixels zeroed-out
        rejected_list   : boolean mask, True for rejected patches
        patch_shapes    : list of (h, w) per patch
        target_mpp      : float
            µm / pixel of *og_img*.  Returned ``coords_list`` will be in µm.
        cores           : int, thread pool size
        """
        begin = time.time()
            ...
            for i in i_range:
                for j in j_range:
                    ...
                    if not rejected_list[idx]:
                        # convert image-space px -> µm so all share one frame of reference
                        coords_list.append(
                            (
                                int(j*patch_shapes[idx][1]*target_mpp / 1.0), 
                                int(i*patch_shapes[idx][0]*target_mpp / 1.0)
                            )
                        )
