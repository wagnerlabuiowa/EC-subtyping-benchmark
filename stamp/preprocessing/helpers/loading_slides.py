...
def get_slide_mpp(slide: openslide.OpenSlide) -> float:
    try:
        ...
        #         slide_mpp = DEFAULT_TIFF_MPP
        #         print(f"MPP re-matched from default after initial failure: {slide_mpp}")
        # except:
...
def get_raw_tile_list(I_shape: tuple, bg_reject_array: np.array, rejected_tile_array: np.array, patch_shapes: np.array, target_mpp: float):
    canny_output_array=[]
    ...
    for i in i_range:
        for j in j_range:
            ...
            if not rejected_tile_array[idx]:
                # old  # coords_list.append((j*patch_shapes[idx][1], i*patch_shapes[idx][0]))
                coords_list.append((
                    int(j * patch_shapes[idx][1] * target_mpp),   # x in µm  (lvl-0 px * µm/px)
                    int(i * patch_shapes[idx][0] * target_mpp)    # y in µm
                ))
